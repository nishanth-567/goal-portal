...
export async function GET(req: NextRequest) {
  ...
  // Get all users (for admin) or direct reports (for manager)
  let userFilter: any = {};
  ...
  const userSummaries = users.map((user) => {
    ...
    const completedCheckins = lockedGoals.filter((g) =>
    ...
    return {
      ...
      completedGoals: completedCheckins,
  ...
  for (const goal of allGoals) {
    const name = goal.thrustArea.name;
    ...
    thrustAreaMap[name].totalScore += scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : 0;
  }
  ...
  const checkinCompletion = periods.map((period) => {
    ...
    return { period, completed, total: totalEmployees, rate: totalEmployees > 0 ? Math.round((completed / totalEmployees) * 100) : 0 };
  });
